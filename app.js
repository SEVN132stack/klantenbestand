    ...
    document.getElementById('detail-content').innerHTML=
      ...
      +'<div class="dgrid">'
        ...
        +'<div class="dcard"><div class="dcard-t">Financien</div>'
          ...
          +(c.facturatie_type?
            '<div class="irow"><span class="ik">Factuurtype</span><span class="iv">'+(c.facturatie_type==='vast_bedrag'?'Vast bedrag':'Uurtarief')+'</span></div>'
            +( c.facturatie_type==='vast_bedrag'?
              '<div class="irow"><span class="ik">Beschikking bedrag</span><span class="iv">'+fmtE(c.beschikking_bedrag)+'</span></div>'
              : (c.uren!==null||c.minuten!==null?
                '<div class="irow"><span class="ik">Tijd</span><span class="iv">'+(c.uren||0)+' uur '+(c.minuten||0)+' min</span></div>'
                :'')
            )
          :'')
        +'</div>'
...
function openModal(id){
  ...
  g('f-facturatie-type',c&&c.facturatie_type);
  g('f-beschikking-bedrag',c&&c.beschikking_bedrag);
  g('f-uren',c&&c.uren);
  g('f-minuten',c&&c.minuten);
  toggleFacturatieType();
...
function saveClient(){
  ...
  var body={naam:naam,bsn:document.getElementById('f-bsn').value||null,
    ...
    laatste_gefactureerd:document.getElementById('f-laatste-gefact').value||null,
    facturatie_type:document.getElementById('f-facturatie-type').value||null,
    beschikking_bedrag:parseFloat(document.getElementById('f-beschikking-bedrag').value)||null,
    uren:parseInt(document.getElementById('f-uren').value)||null,
    minuten:parseInt(document.getElementById('f-minuten').value)||null,
...
function initRapFilters(){
  ...
  if(!klantSel)return;
  var klanten=Array.from(new Set(clients.map(function(c){return c.klant;}).filter(Boolean))).sort();
  var begs=Array.from(new Set(clients.flatMap(function(c){return [c.begeleider_1,c.begeleider_2];}).filter(Boolean))).sort();
  var locs=Array.from(new Set(clients.map(function(c){return c.locatie;}).filter(Boolean))).sort();
  klantSel.innerHTML='<option value="">Alle klanten</option>'+klanten.map(function(k){return '<option>'+k+'</option>';}).join('');
  begSel.innerHTML='<option value="">Alle begeleiders</option>'+begs.map(function(b){return '<option>'+b+'</option>';}).join('');
  locSel.innerHTML='<option value="">Alle locaties</option>'+locs.map(function(l){return '<option>'+l+'</option>';}).join('');
}
...
function renderRapSummary(rows){
  var totB=rows.reduce(function(s,c){return s+(parseFloat(c.bedrag_beschikt)||0);},0);
  var totG=rows.reduce(function(s,c){return s+(parseFloat(c.gefactureerd)||0);},0);
  var totP=rows.reduce(function(s,c){return s+(parseFloat(c.betaald)||0);},0);
  ...
  el.innerHTML=
    ...
    '<div class="rsum-card"><div class="rsum-label">Betaald</div><div class="rsum-val" style="font-size:16px;color:#0F6E56">'+fmtE(totP)+'</div><div class="rsum-sub">'+(totG>0?Math.round(totP/totG*100):0)+'% van gefactureerd</div></div>'+
    '<div class="rsum-card"><div class="rsum-label">Openstaand</div><div class="rsum-val" style="font-size:16px;color:'+(totB-totG>0?'#854F0B':'#0F6E56')+'">'+fmtE(totB-totG)+'</div><div class="rsum-sub">Beschikt min gefactureerd</div></div>';
}
...
function renderRapportage(){
  ...
  if(view==='clienten'){
    thead.innerHTML='<tr><th>Naam</th><th>Status</th><th>Klant</th><th>Begeleider</th><th>Locatie</th><th>Datum start</th><th>Einde beschikking</th></tr>';
    ...
    thead.innerHTML='<tr><th>Naam</th><th>Status</th><th>Klant</th><th>Beschikt</th><th>Gefactureerd</th><th>Betaald</th><th>Openstaand</th><th>% Gefact.</th></tr>';
    if(!rows.length){tbody.innerHTML='<tr><td colspan="8"><div class="empty-state">Geen cliënten gevonden</div></td></tr>';return;}
    tbody.innerHTML=rows.map(function(c){
      var b=parseFloat(c.bedrag_beschikt)||0;
      var g=parseFloat(c.gefactureerd)||0;
      var p=parseFloat(c.betaald)||0;
      var o=b-g;
      var pct=b>0?Math.round(g/b*100):0;
      var kleurO=o>0?'#854F0B':'#0F6E56';
      return '<tr class="row-link" onclick="openDetail(\''+c.id+'\')">'
        +'<td style="font-weight:500">'+c.naam+'</td>'
        +'<td>'+stBadge(c.status)+'</td>'
        +'<td>'+(c.klant||'-')+'</td>'
        +'<td style="text-align:right">'+fmtE(b)+'</td>'
        +'<td style="text-align:right">'+fmtE(g)+'</td>'
        +'<td style="text-align:right">'+fmtE(p)+'</td>'
        +'<td style="text-align:right;color:'+kleurO+'">'+fmtE(o)+'</td>'
        +'<td style="text-align:center">'
          +'<div style="display:flex;align-items:center;gap:6px;justify-content:flex-end">'
            +'<div style="width:50px;height:5px;background:var(--bg3);border-radius:5px;overflow:hidden">'
              +'<div style="width:'+pct+'%;height:100%;background:#185FA5;border-radius:5px"></div>'
            +'</div>'
            +'<span style="font-size:12px;color:var(--t2);min-width:30px">'+pct+'%</span>'
          +'</div>'
        +'</td>'
        +'</tr>';
    }).join('');
    ...
    thead.innerHTML='<tr><th>Naam</th><th>Status</th><th>Begeleider</th><th>Datum start</th><th>Beschikt</th><th>Gefactureerd</th></tr>';
    if(!rows.length){tbody.innerHTML='<tr><td colspan="6"><div class="empty-state">Geen cliënten gevonden</div></td></tr>';return;}
    var html='';
    klanten.forEach(function(klant){
      var sub=rows.filter(function(c){return (c.klant||'Onbekend')===klant;});
      var totB=sub.reduce(function(s,c){return s+(parseFloat(c.bedrag_beschikt)||0);},0);
      html+='<tr style="background:var(--bg2)">'
        +'<td colspan="4" style="font-weight:500;color:var(--t1);padding:8px 13px">'+klant+' <span style="font-size:11px;font-weight:400;color:var(--t2)">('+sub.length+' clienten)</span></td>'
      ...
      sub.forEach(function(c){
        html+='<tr class="row-link" onclick="openDetail(\''+c.id+'\')">'
          ...
          +'<td>'+([c.begeleider_1,c.begeleider_2].filter(Boolean).join(', ')||'-')+'</td>'
          +'<td>'+fmtD(c.datum_start)+'</td>'
          +'<td style="text-align:right">'+fmtE(c.bedrag_beschikt)+'</td>'
          +'<td style="text-align:right">'+fmtE(c.gefactureerd)+'</td>'
          +'</tr>';
      });
    });
...
function renderBeschikkingenInModal(){
  ...
  list.innerHTML = _beschikkingen.map(function(b, idx){
    var bid = b.id || b._id;
    return '<div class="beschikking-item" id="bitem-'+bid+'">'
      ...
      +'<div class="beschikking-grid">'
        ...
        +'<div class="ff2"><label>Bedrag (EUR)</label><input type="number" step="0.01" id="bs-bedrag-'+bid+'" value="'+(b.bedrag||'')+'"></div>'
        +'<div class="ff2"><label>Product</label><input type="text" id="bs-product-'+bid+'" value="'+(b.product||'')+'"></div>'
        +'<div class="ff2"><label>Toewijzingsnummer</label><input type="text" id="bs-toewijzing-'+bid+'" value="'+(b.toewijzingsnummer||'')+'"></div>'
        +'<div class="ff2"><label>Opmerkingen</label><input type="text" id="bs-opm-'+bid+'" value="'+(b.opmerkingen||'')+'"></div>'
      +'</div>'
    +'</div>';
  }).join('');
}
...
function getBeschikkingenFormData(){
  return _beschikkingen.map(function(b){
    ...
    return {
      _id: b.id || null,
      _tmp: b._tmp || false,
      datum_start: document.getElementById('bs-start-'+bid)&&document.getElementById('bs-start-'+bid).value || null,
      datum_einde: document.getElementById('bs-einde-'+bid)&&document.getElementById('bs-einde-'+bid).value || null,
      bedrag: parseFloat(document.getElementById('bs-bedrag-'+bid)&&document.getElementById('bs-bedrag-'+bid).value) || null,
      product: document.getElementById('bs-product-'+bid)&&document.getElementById('bs-product-'+bid).value || null,
      toewijzingsnummer: document.getElementById('bs-toewijzing-'+bid)&&document.getElementById('bs-toewijzing-'+bid).value || null,
      opmerkingen: document.getElementById('bs-opm-'+bid)&&document.getElementById('bs-opm-'+bid).value || null,
    };
  });
}
...
function slaanBeschikkingenOp(clientId){
  ...
  bestaand.forEach(function(b){
    var id=b._id;
    var body={datum_start:b.datum_start,datum_einde:b.datum_einde,bedrag:b.bedrag,product:b.product,toewijzingsnummer:b.toewijzingsnummer,opmerkingen:b.opmerkingen};
    promises.push(api('PUT','/clienten/'+clientId+'/beschikkingen/'+id,body));
  });
  nieuw.forEach(function(b){
    var body={datum_start:b.datum_start,datum_einde:b.datum_einde,bedrag:b.bedrag,product:b.product,toewijzingsnummer:b.toewijzingsnummer,opmerkingen:b.opmerkingen};
    promises.push(api('POST','/clienten/'+clientId+'/beschikkingen',body));
  });
  ...
  beschikkingen.forEach(function(b){
    html+='<div class="beschikking-item">'
      ...
      +'<div class="beschikking-grid">'
        ...
        +'<div class="ff2"><span class="fin-label">Bedrag</span><div style="font-size:13px;font-weight:500;color:var(--t1);margin-top:2px">'+fmtE(b.bedrag)+'</div></div>'
        +'<div class="ff2"><span class="fin-label">Product</span><div style="font-size:13px;font-weight:500;color:var(--t1);margin-top:2px">'+(b.product||'-')+'</div></div>'
        +(b.toewijzingsnummer?'<div class="ff2"><span class="fin-label">Toewijzingsnummer</span><div style="font-size:13px;color:var(--t1);margin-top:2px">'+b.toewijzingsnummer+'</div></div>':'')
        +(b.opmerkingen?'<div class="ff2 full"><span class="fin-label">Opmerkingen</span><div style="font-size:13px;color:var(--t1);margin-top:2px">'+b.opmerkingen+'</div></div>':'')
      +'</div>'
    +'</div>';
  });
  html+='</div>';
