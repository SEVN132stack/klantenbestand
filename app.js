...
function _renderRapTabel(view, rows, thead, tbody){
  if(view==='clienten'){
    thead.innerHTML='<tr><th>Naam</th><th>Status</th><th>Klant</th><th>Begeleider</th><th>Datum start</th><th>Einde beschikking</th><th>Beschikkingen</th><th>Beschikt</th></tr>';
    if(!rows.length){tbody.innerHTML='<tr><td colspan="8"><div class="empty-state">Geen clienten gevonden</div></td></tr>';return;}
    tbody.innerHTML=rows.map(function(c){
      var bs=_rapBeschikkingen[c.id]||[];
      var t=getBeschikkingenTotalen(c.id);
      var bLabel=bs.length
        ?bs.map(function(b,i){return 'B'+(i+1)+': '+fmtD(b.datum_start)+' - '+fmtD(b.datum_einde);}).join(' | ')
        :'-';
      return '<tr class="row-link" onclick="openDetail(\''+c.id+'\')">'        +'<td style="font-weight:500">'+c.naam+'</td>'        +'<td>'+stBadge(c.status)+'</td>'        +'<td>'+(c.klant||'-')+'</td>'        +'<td>'+([c.begeleider_1,c.begeleider_2].filter(Boolean).join(', ')||'-')+'</td>'        +'<td>'+fmtD(c.datum_start)+'</td>'        +'<td>'+fmtD(c.einde_beschikking)+'</td>'        +'<td style="font-size:12px;color:var(--t2)">'+bLabel+'</td>'        +'<td style="text-align:right">'+(t.beschikt>0?fmtE(t.beschikt):'-')+'</td>'        +'</tr>';
    }).join('');
  }
    ...
    if(!rows.length){tbody.innerHTML='<tr><td colspan="9"><div class="empty-state">Geen clienten gevonden</div></td></tr>';return;}
    tbody.innerHTML=rows.map(function(c){
      var t=getBeschikkingenTotalen(c.id);
      var o=t.beschikt-t.gefact;
      var pct=t.beschikt>0?Math.round(t.gefact/t.beschikt*100):0;
      var kleurO=o>0?'#854F0B':'#0F6E56';
      return '<tr class="row-link" onclick="openDetail(\''+c.id+'\')">'        +'<td style="font-weight:500">'+c.naam+'</td>'        +'<td>'+stBadge(c.status)+'</td>'        +'<td>'+(c.klant||'-')+'</td>'        +'<td style="text-align:center;color:var(--t2)">'+t.aantal+'</td>'        +'<td style="text-align:right">'+(t.beschikt>0?fmtE(t.beschikt):'-')+'</td>'        +'<td style="text-align:right">'+(t.gefact>0?fmtE(t.gefact):'-')+'</td>'        +'<td style="text-align:right;color:#0F6E56">'+(t.betaald>0?fmtE(t.betaald):'-')+'</td>'        +'<td style="text-align:right;color:'+kleurO+'">'+(t.beschikt>0?fmtE(o):'-')+'</td>'        +'<td style="text-align:center">'          +'<div style="display:flex;align-items:center;gap:4px;justify-content:flex-end">'            +'<div style="width:40px;height:5px;background:var(--bg3);border-radius:5px;overflow:hidden">'              +'<div style="width:'+pct+'%;height:100%;background:#185FA5;border-radius:5px"></div>'            +'</div>'            +'<span style="font-size:12px;color:var(--t2)">'+pct+'%</span>'          +'</div>'        +'</td>'        +'</tr>';
    }).join('');
  }
  else if(view==='per-klant'){
    var klanten=Array.from(new Set(rows.map(function(c){return c.klant||'Onbekend';}))).sort();
    thead.innerHTML='<tr><th>Naam</th><th>Status</th><th>Begeleider</th><th>Datum start</th><th># Besch.</th><th>Beschikt</th><th>Gefactureerd</th><th>Betaald</th></tr>';
    if(!rows.length){tbody.innerHTML='<tr><td colspan="8"><div class="empty-state">Geen clienten gevonden</div></td></tr>';return;}
    var html='';
    klanten.forEach(function(klant){
      var sub=rows.filter(function(c){return (c.klant||'Onbekend')===klant;});
      var totB=0,totG=0,totP=0,totA=0;
      sub.forEach(function(c){var t=getBeschikkingenTotalen(c.id);totB+=t.beschikt;totG+=t.gefact;totP+=t.betaald;totA+=t.aantal;});
      html+='<tr style="background:var(--bg2)">'        +'<td colspan="4" style="font-weight:500;color:var(--t1);padding:8px 13px">'+klant+' <span style="font-size:11px;font-weight:400;color:var(--t2)">('+sub.length+' clienten, '+totA+' beschikkingen)</span></td>'        +'<td style="text-align:center;padding:8px 13px;font-weight:500;color:var(--t2)">'+totA+'</td>'        +'<td style="text-align:right;padding:8px 13px;font-weight:500">'+(totB>0?fmtE(totB):'-')+'</td>'        +'<td style="text-align:right;padding:8px 13px;font-weight:500">'+(totG>0?fmtE(totG):'-')+'</td>'        +'<td style="text-align:right;padding:8px 13px;font-weight:500;color:#0F6E56">'+(totP>0?fmtE(totP):'-')+'</td>'        +'</tr>';
      sub.forEach(function(c){
        var t=getBeschikkingenTotalen(c.id);
        html+='<tr class="row-link" onclick="openDetail(\''+c.id+'\')">'          +'<td style="font-weight:500;padding-left:24px">'+c.naam+'</td>'          +'<td>'+stBadge(c.status)+'</td>'          +'<td>'+([c.begeleider_1,c.begeleider_2].filter(Boolean).join(', ')||'-')+'</td>'          +'<td>'+fmtD(c.datum_start)+'</td>'          +'<td style="text-align:center;color:var(--t2)">'+t.aantal+'</td>'          +'<td style="text-align:right">'+(t.beschikt>0?fmtE(t.beschikt):'-')+'</td>'          +'<td style="text-align:right">'+(t.gefact>0?fmtE(t.gefact):'-')+'</td>'          +'<td style="text-align:right;color:#0F6E56">'+(t.betaald>0?fmtE(t.betaald):'-')+'</td>'          +'</tr>';
      });
    });
...
function slaanBeschikkingenOp(clientId){
  ...
  return Promise.all(promises).then(function(){
    // Cache legen voor deze client zodat financieel overzicht opnieuw laadt
    delete _rapBeschikkingen[clientId];
    return api('GET','/clienten/'+clientId+'/beschikkingen').then(function(bs){
      _rapBeschikkingen[clientId] = bs||[];
      renderFinancieel();
    });
  });
}
...
function verwijderBeschikkingDossier(clientId, beschikkingId){
  ...
  api('DELETE','/clienten/'+clientId+'/beschikkingen/'+beschikkingId).then(function(){
    delete _rapBeschikkingen[clientId];
    api('GET','/clienten/'+clientId+'/beschikkingen').then(function(bs){
      _rapBeschikkingen[clientId] = bs||[];
      renderFinancieel();
    });
    openDetail(clientId);
