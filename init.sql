...
CREATE TABLE clienten (
    ...
    bsn             VARCHAR(9),
    geboortedatum   DATE,
    status          VARCHAR(50) NOT NULL DEFAULT 'Aangemeld',
    klant           VARCHAR(150),
    locatie         VARCHAR(100),
    ...
    laatste_gefactureerd VARCHAR(20),
    facturatie_type     VARCHAR(20),
    beschikking_bedrag  NUMERIC(10,2),
    uren                INT,
    minuten             INT,
    opmerkingen     TEXT,
    notitie         TEXT,
...
-- Wachtwoord wordt ingesteld via de backend bij eerste start
-- Voer na "docker compose up" dit commando uit om een admin aan te maken:
...
-- ─── Beschikkingen ────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS beschikkingen (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id       UUID NOT NULL REFERENCES clienten(id) ON DELETE CASCADE,
    volgnummer      INT NOT NULL DEFAULT 1,
    datum_start     DATE,
    datum_einde     DATE,
    bedrag          NUMERIC(10,2),
    product         VARCHAR(100),
    toewijzingsnummer VARCHAR(100),
    opmerkingen     TEXT,
    aangemaakt      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_beschikkingen_client ON beschikkingen(client_id);
